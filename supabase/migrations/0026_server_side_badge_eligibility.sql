-- sync_badges() (0016) validated that a submitted badge KEY exists in the
-- catalogue and pinned its XP payout there, but never verified the calling
-- child's data actually satisfies that badge's criteria -- eligibility was
-- computed entirely client-side (BADGES[].check() in src/lib/gamification.js)
-- and simply trusted. A child with devtools could call sync_badges with all
-- ~84 known keys at once and be instantly awarded every badge and ~50,000+ XP
-- (Level 35 / "FinnTrack Legend") regardless of actual activity.
--
-- Impact was cosmetic only -- XP/level/badges gate no money, no permissions,
-- and nothing else in the app reads them for access control -- but this
-- closes the gap completely by moving eligibility computation itself into
-- Postgres, so the server is the sole source of truth for both WHICH badges
-- are earned and WHAT they pay out. sync_badges() no longer accepts a client
-- badge list at all; the old sync_badges(text[]) is dropped outright so the
-- vulnerable entry point can't be called even directly.
--
-- finn_evaluate_badges() is a line-by-line SQL port of buildAchievementContext
-- + evaluateBadges() in src/lib/gamification.js. Keep the two in sync: a
-- badge's check() changing there without a matching change here just means
-- the server silently disagrees with the client's now-purely-cosmetic local
-- preview (see achievementsSync.js, which no longer trusts its own
-- computation and only displays whatever this function decides).
--
-- One known, accepted parity gap: "Same Day" and "Quick Logger" compare a
-- transaction's claimed local date/time against its server created_at
-- instant. The client did this in the browser's own timezone; the server has
-- no stored per-child timezone, so it approximates by treating the claimed
-- date/time as if it were UTC. This can award (or withhold) those two
-- specific badges up to a day off from what the client would have shown, for
-- children outside UTC. No other badge is affected -- everything else
-- compares same-typed local values (date to date) or absolute instants
-- (created_at to created_at / now()), which have no timezone ambiguity.
--
-- Not granted to authenticated/anon -- callable only from within sync_badges,
-- same pattern as badge_catalogue itself (see 0016): it is SECURITY DEFINER
-- and reads whatever child_id it's given with no ownership check of its own,
-- so exposing it directly would let any authenticated user read (though not
-- alter) any other child's aggregated activity stats.
...
begin
  ...
  select case when count(*) > 1 then
    exists (
      select 1 from generate_series(min(wk) + 7, date_trunc('week', current_date)::date - 7, 7) as gs(wk)
      where gs.wk not in (select wk from weeks)
    )
  else false end
    ...
    select c.*,
      ...
      (
        select count(*)::int from generate_series(
          make_date(c.yr, c.mo, 1),
          (make_date(c.yr, c.mo, 1) + interval '1 month' - interval '1 day')::date,
          interval '1 day'
...
-- Only a child account can earn badges (unchanged from 0016). Eligibility is
-- now fully server-computed -- the caller supplies nothing but their own
-- identity via auth.uid().
drop function if exists public.sync_badges(text[]);
...
begin
  ...
  select coalesce(array_agg(badge_key), '{}') into v_already
  from public.user_badges where child_id = v_uid;

  v_full := public.finn_evaluate_badges(v_uid, v_already);

  select coalesce(array_agg(k), '{}') into v_new
  from unnest(v_full) as k
  where not (k = any(v_already));

  foreach v_key in array v_new loop
    select c.tier into v_tier from public.badge_catalogue c where c.badge_key = v_key;
    if v_tier is null then
      continue; -- defensive: finn_evaluate_badges only ever emits catalogue keys
    end if;
